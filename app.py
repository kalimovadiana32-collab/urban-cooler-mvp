...
st.markdown("""
    ...
    /* Делаем текст белым, но не ломаем внутренности полей ввода */
    h1, h2, h3, h4, h5, h6, .stMarkdown p, label, .stText {
        color: #ffffff !important;
    }

    /* Карточки инструкций - темный фон, чтобы белый текст читался */
    .guide-card {
    ...
    /* Алерт-панели - темный фон для контраста */
    ...
    /* Исправление проблемы "белое на белом" в таблицах */
    table, th, td { background-color: #1e2530 !important; color: #ffffff !important; border-color: #333 !important; }
    thead tr th { background-color: #2c3643 !important; color: #00ff88 !important; }
    
    /* Исправление виджетов (селекторы и инпуты) */
...
# --- ОБШИРНАЯ ИНСТРУКЦИЯ (С МАСШТАБОМ 20м) ---
with st.expander("📖 ИНСТРУКЦИЯ ПО РАБОТЕ С ПЛАТФОРМОЙ (Нажмите, чтобы развернуть)", expanded=True):
    ...
    **Как получить точный результат:**
    1. **Подготовка снимка:** Откройте Яндекс/Google Карты, переключитесь в режим "Спутник" (строго 2D, вид сверху). **ОБЯЗАТЕЛЬНО установите масштаб 20м** (найдите линейку масштаба в правом нижнем углу карты и приблизьте до значения 20м). Сделайте скриншот проблемного района.
    ...
    3. **Выделение зоны (Кроппинг):** С помощью зеленой рамки выделите конкретный квартал для анализа. При масштабе 20м система берет за основу расчетную площадь ~10 Гектаров.
...
uploaded_file = st.file_uploader("📥 Загрузите спутниковый снимок (Масштаб 20м, формат JPG/PNG)", type=['jpg', 'png', 'jpeg'])

if uploaded_file:
    img_raw = auto_enhance_image(Image.open(uploaded_file))
    st.write("### 🎯 2. Выделение зоны анализа")
    st.markdown("*Растяните зеленую рамку на нужный квартал (расчеты ведутся из предположения, что зона = 10 Га при масштабе 20м)*")
